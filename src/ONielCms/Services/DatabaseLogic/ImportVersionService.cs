...
namespace ONielCms.Services.DatabaseLogic
{
    ...
    public class ImportVersionService : IImportVersionService
    {
        ...
        {
            if (string.IsNullOrEmpty(model.Data.Version)) throw new Exception("Model.Data.Version is null or empty!");
        ...
        public Task RemoveAllVersions()
        {
            return m_storageContext.MakeInTransaction(
                async () =>
                {
                    await m_storageContext.Delete<ResourceVersion>(new Query());
                    await m_storageContext.Delete<RouteResource>(new Query());
                    await m_storageContext.Delete<Resource>(new Query());
                    await m_storageContext.Delete<RouteVersion>(new Query());
                    await m_storageContext.Delete<RouteEntity>(new Query());
                    await m_storageContext.Delete<Edition>(new Query());
                }
            );
        }
